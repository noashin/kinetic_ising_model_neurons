...
def calc_log_evidence(a, b, nu, s, mu, m, v, ro, N):
    ...
    B = np.sum(np.multiply(mu**2, nu / 1)) - np.sum(np.multiply(v**(-1), m**2))
    log_C = np.sum(ro * a + (1-ro) * b)

    log_evidence = log_C + np.log(2.0 * np.pi) * N / 2.0 + np.sum(0.5 * np.log(nu)) + B / 2 + np.sum(s)
